...
**Global Forest Watch (GFW)** is the most comprehensive online tool for monitoring the [world’s forest cover](http://www.globalforestwatch.org/). It analyses satellite imagery for forest loss in near real-time, provides alert systems for deforestation and fires and sends emails when a selected area undergoes changes. GFW also shows the latest satellite imagery from Landsat 8 and Sentinel-2, updating as soon as new imagery is available.
